...
// Parity
#define ARM_USART_PARITY_NONE               0U      ///< No Parity (default)
#define ARM_USART_PARITY_ODD                1U     ///< Odd Parity
#define ARM_USART_PARITY_EVEN               2U      ///< Even Parity
...
typedef struct _ARM_DRIVER_USART
{
	...
	int32_t (*SetData)(USART_Config_t* config);
	int32_t (*SetParity)(USART_Config_t* config);
	int32_t (*SetDirection)(USART_Config_t* config);
	int32_t (*Configstopbit)(USART_Config_t* config);
	int32_t (*Transmit)(const void *data, uint32_t num);
